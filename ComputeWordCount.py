...
# positional argument#
# #####################
parser.add_argument("--topic_file", default=r'D:\613\TopicModel\outputs\20News_output\DRvae_sp_beta_selector\DRvae_sp_beta_selector20\DRvae_sp_beta_selector20_1.topWords',
                    help="file that contains the topics")
parser.add_argument("--ref_corpus_dir", default="D:/613/ref_corpus/wiki2", help="directory that contains the reference corpus")
...
def calc_word_count(words, topic_word_rel, unigram_list, worker_wordcount):
    ...
    # ngrams = [unigram_list[words[idx] - 1] for idx in np.nonzero(words)[0]]
...
def calcwcngram(worker_num, window_size, corpus_file, topic_word_rel, unigram_list, unigram_rev):
    ...
    for line in codecs.open(corpus_file, "r", "utf-8"):
        ...
        # for head_id in range(num_windows):
        #     if window_size != 0:
        #         words_in_window = words[head_id:head_id + window_size]
        #     else:
        #         words_in_window = words
...
# vocab_words = [l.strip() for l in lines]
# # update the unigram list and topic word relation
...
# wordco0 = calcwcngram(0,window_size, corpus_partitions[0], topic_word_rel, unigram_list, unigram_rev)
# calcwcngram_complete(wordco0)
